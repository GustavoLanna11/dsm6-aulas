...
class GatewayIndisponivelError(GatewayPagamentoError):
    """Exceção quando o gateway está indisponível."""
    pass

class GatewayPagamentoClient:
    """Cliente HTTP para comunicação com o gateway de pagamento."""
    ...
    def processar_transacao(
        ...
        try:
            with httpx.Client(base_url=self.base_url, timeout=self.timeout) as 
            client:
                ...
                if response.status_code == 200;
                    ...
                    return {
                        "aprovado": False,
                        "status": "RECUSADO",
                        "motivo": corpo.get("motivo", "Transação recusada pelo gateway"),
                        "transacao_id": None,
                    }
                else:
            ...
            raise GatewayIndisponivelError("Gateway de pagamento indisponível") from exc